...
cd /cyberpunk/wifi && git clone $GIT_URL
pip${DAS_PYTHON_VERSION} install $DAS_BUILD_PIP_DEPS
apt install $DAS_BUILD_DEPS -y
cd /cyberpunk/wifi/wifiphisher && python${DAS_PYTHON_VERSION} setup.py build
cd /cyberpunk/wifi/wifiphisher && python${DAS_PYTHON_VERSION} setup.py install
